...
<p>Imagine dropping a pebble into a still pond. The ripples spread outward in perfect rings, reaching every point at distance 1 first, then distance 2, and so on — never jumping ahead. BFS (Breadth-First Search) works exactly like those ripples. Starting from a source node it visits all neighbors before moving to neighbors-of-neighbors, processing the graph <strong>level by level</strong>. A queue is the natural data structure because the first node added to a level is the first one whose neighbors we explore, preserving the ripple order.</p>
<p>This level-by-level expansion is precisely why BFS is the correct algorithm for <strong>shortest paths in unweighted graphs</strong>. The moment BFS first reaches a node, it has found the minimum number of hops to get there. Any later path to that same node must be at least as long, so the first visit is optimal. This guarantee breaks down in weighted graphs (use Dijkstra instead) and does not apply to DFS, which can find a node via a long winding path before discovering a shorter route.</p>
<p>Reach for BFS whenever a problem asks for the <em>fewest steps</em>, <em>minimum moves</em>, or <em>shortest transformation sequence</em> between two states in an unweighted space — grids, word ladders, lock combinations, and social network degrees all fit this mold. The most common mistake is forgetting to mark a node as visited <em>when it is added to the queue</em> rather than when it is dequeued; doing the latter allows the same node to be enqueued multiple times, causing redundant work and, in the worst case, infinite loops.</p>
<div class="alert tip"><span class="alert-icon">💡</span><strong>Key insight:</strong> BFS guarantees the shortest path in unweighted graphs because it explores all paths of length <em>k</em> before exploring any path of length <em>k+1</em>. Mark nodes visited on enqueue, not on dequeue, to keep the time complexity at <code>O(V+E)</code>.</div>
